...
def minmax(x):
        ...
        return(x)
bulk = minmax(pd.read_csv('/lustre/wangjc01/huzixin/deconv/data/bulk_lmy.csv'))
ref = minmax(pd.read_csv('/lustre/wangjc01/huzixin/deconv/data/ref_lmy.csv'))
cdata = np.concatenate((bulk,ref),axis=0)
#argv = ['test.py','ae',17121,1024,200,128,50,1]
argv = sys.argv
if argv[1] == 'ae':
  model = ae
elif argv[1] == 'vae':
...
encoder.save('/lustre/wangjc01/huzixin/deconv/log/rlt_lmy/%s.encoder' % fo)
decoder.save('/lustre/wangjc01/huzixin/deconv/log/rlt_lmy/%s.decoder' % fo)
pd.DataFrame(bulk_encoded).to_csv('/lustre/wangjc01/huzixin/deconv/log/rlt_lmy/%s.bulk_encoded'%fo,index=0)
pd.DataFrame(ref_encoded).to_csv('/lustre/wangjc01/huzixin/deconv/log/rlt_lmy/%s.ref_encoded'%fo,index=0)
fo = open('/lustre/wangjc01/huzixin/deconv/log/rlt_lmy/%s.rlt' % fo, "w")
for k in history:
        fo.write('%s: %s\n' % (k, history[k]))
fo.close()
